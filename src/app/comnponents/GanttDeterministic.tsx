...
type Activity = {
  name: string;
  precedence: string[];
  duration: number; // Utilizamos duración directamente en modo determinístico
  totalFloat: number;
};

interface GanttDeterministicProps {
  activities: Activity[];
}
...
const GanttDeterministic: React.FC<GanttDeterministicProps> = ({ activities }) => {
    ...
    while (!allDatesAdjusted) {
      ...
      activityDates = activityDates.map((activity) => {
        if (activity.dependencies) {
          ...
          const duration = activities.find((act) => act.name === activity.id)?.duration ?? 0;
...
export default GanttDeterministic;
